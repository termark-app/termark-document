...
至于像 [Termark](https://termark.app) 这样的 SSH/SFTP 客户端，它不是要取代 `tmux`、`systemd` 或者任务队列——它能做的是让你连上服务器，然后用服务器上这些工具去管好任务：通过 SSH 连接目标主机，在终端里创建或重新接入 `tmux` 会话，查看日志、进程和 `systemd` 状态，需要的时候通过 SFTP 传脚本、下日志，断开之后再连回来接着看结果。客户端负责远程访问，任务生命周期还是交给服务器上的这些工具——分清楚这条边界，出问题的时候才知道该查连接、查会话，还是查服务配置。
