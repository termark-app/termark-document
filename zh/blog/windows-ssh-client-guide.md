...
![Termark Windows 客户端的创建主机窗口，包含 SSH 协议、服务器地址、22 端口、认证方式和连接方式设置](./images6/windows-new-ssh-host.png)

*在 Windows 版 Termark 中创建 SSH 主机：可选择手动输入或已保存凭证，并配置直接连接、SSH 跳板或代理。截图中的地址和用户名均为界面占位示例。*
