...
**密钥密码要不要设？** 强烈建议设置。没有密码保护的私钥一旦文件泄露，攻击者可以直接使用；有密码的私钥至少还需要破解密码这一步。虽然密码会增加每次连接的输入成本，但配合 ssh-agent 只需输入一次即可缓解。对于自动化场景，可以用 ssh-agent 的 `IdentityAgent` 配置指向专用的 agent socket，避免在脚本中硬编码明文密码。

**`authorized_keys` 里公钥太多怎么清理？** 定期审计是关键。建议每季度执行一次全量指纹扫描，对比上一次的记录，找出长期未使用的公钥。对于离职员工的密钥，应在离职当天删除，而不是等到"有空再处理"。可以用 `last` 命令查看各用户最近的登录时间，辅助判断哪些密钥可能已不再使用。

**云厂商的 SSH 密钥和自生成的有什么区别？** 云厂商（阿里云、AWS、腾讯云）提供的密钥对功能本质上是在创建实例时自动将公钥写入 `~/.ssh/authorized_keys`，之后的管理方式与自生成密钥完全一致。云厂商只保存公钥，私钥由你下载保管。如果私钥泄露，即使通过云控制台重新注入新密钥，旧密钥如果已被复制到其他机器上仍然有效——所以轮换时务必手动清理所有服务器上的旧公钥，不能只依赖云控制台操作。

## 结语

SSH 密钥管理不是一次性工作。密钥生成用 ed25519，管理靠权限控制和 ssh-agent，轮换按"先部署后撤销"的顺序执行，批量操作交给 Ansible，日常审计用 ssh-audit 和日志分析。把这五步形成 SOP，下次出事时不至于手忙脚乱。Termark 作为 SSH 连接和密钥管理工具，可以帮你简化多台服务器的密钥分发和连接测试——但工具只是辅助，核心在于你自己建立的管理流程。没有流程的工具只是花哨的终端，有流程的工具才是真正的效率提升。
